...
export class FirehoseSubscription extends FirehoseSubscriptionBase {
  ...
  async loadConfig(): Promise<void> {
    ...
    this.SELECTIVE_PHRASES = process.env.FETCH_SELECTIVE_PHRASES?.split(',') || [];
  ...
  async handleEvent(evt: RepoEvent) {
    ...
    const postsToCreate = ops.posts.creates
      .filter((create) => {
        ...
        return this.SUPER_ACCOUNTS.includes(create.author) || this.filterPostByAuthorAndEmojis(create.record.text, create.author);
        //
      })
      .map((create) => {
  ...
  private filterPostByAuthorAndEmojis(text: string, author: string): boolean {
    return this.SELECTIVE_ACCOUNTS.includes(author) && (this.filterTextForEmojis(text, this.SELECTIVE_EMOJIS) || this.filterTextForPhrases(text, this.SELECTIVE_PHRASES));
  }
